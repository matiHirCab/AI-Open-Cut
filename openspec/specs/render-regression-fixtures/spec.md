...
### Requirement: Native static rules-screen lifecycle fixture
The suite MUST include a separate reproducible static rules-screen fixture containing three outlined cards, accent bars, corner brackets, a diagonal procedural grid, concentric circles and layered literal EVERY./SINGLE./ONE. text using existing native shapes/grids/text without external graphic panels. Its recipe MUST record exact local coordinates, dimensions, paints, text/font hash, ordering, 1920x1080 canvas, 10 fps, one-second duration and 0/500/900 ms samples. It MUST retain independent semantic expectations, a synthetic local audio tone and a fresh-directory generation command that refuses overwrite. Existing fixtures and contracts MUST remain unchanged.

#### Scenario: F1 Construct and edit through existing surfaces
- **WHEN** existing typed standalone operations and aliased timeline_batch_edit operations create and edit the scene, including through MCP
- **THEN** all named motifs have independently expected geometry/style/order, batches commit one revision and the scene needs no external graphic panel

#### Scenario: F2 Preserve edits and rollback failures
