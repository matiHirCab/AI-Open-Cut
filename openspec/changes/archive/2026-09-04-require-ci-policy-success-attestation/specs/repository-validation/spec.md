...
- **THEN** the aggregate status still executes, logs all results and the absent or non-authoritative attestation, and fails

#### Scenario: Policy failure is masked by ignored-failure configuration
- **WHEN** the OpenSpec policy command fails but GitHub reports its job result as `success` because failure was ignored
- **THEN** the completion attestation is absent and the aggregate fails even when both functional parity statuses report `success`

#### Scenario: Policy execution is skipped or neutralized
- **WHEN** the OpenSpec policy step does not execute its reviewed command through successful completion
...
The repository's pinned validation workflow MUST structurally verify the stable OpenSpec policy and parity job identities, dependency relationship, unconditional aggregate execution, explicit prerequisite-result and policy-attestation assertions, exact closed OpenSpec, contract, and render step sequences, exact approved properties and environments for every protected step, declared working directories, absence of workflow-level and protected-job-level environment maps, absence of golden mutation or alternate-capture modes, absence of protected-job command defaults and containers, default failure propagation, strict report validation before publication, and publication of the exact validated report path. The policy completion attestation MUST be emitted only after the authoritative validation command succeeds and MUST be exposed and consumed through exact reviewed output bindings. Environment maps on unrelated jobs MUST remain permitted.

#### Scenario: Validate policy completion proof
- **WHEN** the policy step has its exact identifier, runs the authoritative command before emitting the exact success output, exposes only the reviewed job output, and the aggregate binds and checks that output
- **THEN** the CI gate policy check accepts the completion-proof structure

#### Scenario: Detect missing or forged completion proof
- **WHEN** the policy step identifier, command order, emitted value, output destination, job output map, aggregate binding, or exact-success assertion is absent, altered, extended, or replaced
- **THEN** the CI gate policy check fails before the workflow change can be accepted

#### Scenario: Detect ignored policy failure despite successful job conclusion
- **WHEN** step-level or job-level ignored-failure configuration allows a failing policy command to yield a successful job result without its completion output
- **THEN** structural policy validation rejects the configuration and the aggregate model independently rejects the missing attestation

#### Scenario: Validate the required gate structure
- **WHEN** repository validation reads a workflow containing every required job, exact positional protected step, approved property and environment, result and attestation assertion, failure-propagation rule, validation order, and report publication rule without inherited protected-job environment
...
- **WHEN** the OpenSpec job or any of its steps ignores failures, becomes conditional, uses a custom shell, inherits run defaults, runs in a container, or changes its authoritative command, attestation, output, or step properties
