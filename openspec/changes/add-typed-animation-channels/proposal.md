...
The owning model, validation, persistence, and evaluator are in `crates/editor-core`; `apps/headless` and `apps/agent-bridge` translate the typed operation without duplicating validation. `contracts/` owns shared fixtures and operation/capability catalogs. Documentation and tests must cover standalone and batch operations, invalid and missing references, revision conflicts, rollback, history, migration, reopen, and render parity. No new external dependency is expected.
