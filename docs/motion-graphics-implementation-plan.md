...
### M0 EvaluatedScene implementation boundary

MG-M0-03 implements the first owned, renderer-neutral `EvaluatedScene` inside `editor-core`. It evaluates current flat media, text, solid-color, and rectangle items into logical media resources, stable track/item layer order, half-open integer-millisecond spans, typed transforms/keyframes/text styling/transitions, separate audio instructions with resolved mute, fades, volume automation, roles, and ducking settings, and one deterministically merged scene-level voiceover interval table shared by all ducked layers. It accepts at most 4,096 visual layers, 4,096 logical media resources, 4,096 audio layers, 4,096 emitted transition endpoint facts, 10,000 keyframes per property channel, and 10,000 positive voiceover activity ranges before merging. Referenced assets are checked first; video and audio source ends then use checked arithmetic and cannot exceed known asset duration, while images retain their existing ignored-source-offset behavior. Every named limit is preflighted before output or scene-table allocation, with activity ranges counted from per-item temporary data already bounded by the keyframe limit. Transition endpoints are indexed once in declaration order and equal source/target endpoints retain both direction facts.

The scene contains logical asset and generated font-resource IDs only—never paths, URLs, FFmpeg expressions, prepared files, backend commands, or artifact destinations. A private `SceneResourceBindings` sidecar preserves deterministic first-use media path requests and configured font path/family requests without placing them in the scene. A read-only pre-workspace media phase validates binding closure, derives input indexes, canonicalizes media, and rejects project-root escapes before collision inspection, temporary-name allocation, workspace creation, writes, or process execution; the resulting prepared envelope is reused by render preparation. Unconfigured text keeps renderer-controlled default-font selection. The envelope is process-local and does not change schema version 6, migrations, batch aliases, revisions, or retained history. `contracts/motion-graphics-v1.json` therefore remains `fixture_only`.

MG-M0-04 (#13) routes frame preview, audiovisual range preview, materialized draft preview, and final export through that owned scene and sidecar, then retires the borrowed project-backed compatibility traversal. Output intent clips one common scene rather than re-evaluating timeline semantics. Ready renderers expose `evaluated_scene_rendering` through additive protocol-version-1 capability reporting. Exact non-empty semantic-plan parity is authoritative; fixed synthetic frame-preview, range-preview, and export artifacts require SSIM `>= 0.99`, aligned decoded float-PCM RMS error `<= 0.0001`, and timing alignment within one output video frame. Linux CI runs the focused native checks with explicit FFmpeg, FFprobe, and deterministic font paths.
...
Issue #15 advances that observation to schema 3 with separate frame-preview, audiovisual-range-preview, and final-export measurements for scene evaluation, native rasterization, filter construction, decode-only, composite-to-null, encoded output, end-to-end latency, stage work counts, and peak process-tree memory. Decode, composite, and encode are direct overlapping workloads and are explicitly non-additive. The process-tree sampler is scoped to a measured capture and always signals and joins its worker on completion, error, or panic. Fixture migration is limited to the exact revision-2/schema-2 predecessor after complete manifest and report validation; malformed legacy generations are neither migration inputs nor cleanup targets. The current native-raster work count and duration are zero until the planned graphics backend exists; FFmpeg text rendering remains composition work. No public telemetry surface or release threshold is introduced.
