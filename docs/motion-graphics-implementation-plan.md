...
The scene contains logical asset and generated font-resource IDs only—never paths, URLs, FFmpeg expressions, prepared files, backend commands, or artifact destinations. A private `SceneResourceBindings` sidecar preserves deterministic first-use media path requests and configured font path/family requests without placing them in the scene; path-safe preparation remains responsible for canonicalizing and root-checking requests before backend use, while unconfigured text keeps renderer-controlled default-font selection. The envelope is process-local and does not change schema version 6, public contracts, capabilities, migrations, batch aliases, revisions, or retained history. `contracts/motion-graphics-v1.json` therefore remains `fixture_only`. MG-M0-04 (#13) is the separate routing milestone that will switch frame, range, draft, and export planning to consume the owned scene and bindings directly and retire the temporary compatibility traversal.
