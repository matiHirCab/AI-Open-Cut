...
- `Motion-graphics foundation parity` (`foundation-parity`) always runs after OpenSpec policy validation, contract, render, rules-screen, three-platform correctness, and packaged smoke reach terminal results. It logs all six results plus the policy completion attestation, and fails unless every result is `success`, the attestation is `true`, and the duration assertion passes. Maintainers should configure this aggregate status as the required branch-protection check after the workflow is present on the default branch.

The protected workflow has a 120-minute default elapsed-time budget, measured from the Actions API `run_started_at` timestamp to the final foundation duration assertion. That assertion runs even if an earlier foundation check fails, reports the elapsed minutes and budget, and fails closed if the API or timestamp is unavailable. The measured PR #124 candidate run took 2h03m39s because the 1920x1080 rules-screen shard dominated the critical path. A CODEOWNER-reviewed exception allows at most 135 minutes until 2026-10-26; the workflow records its owner, cause, baseline, and [run evidence](https://github.com/matiHirCab/AI-Open-Cut/actions/runs/36244746913). After expiry, remove it or renew it through an approved OpenSpec change with fresh measurements. Required jobs have explicit 135-minute timeouts; foundation has a 10-minute timeout. Queue time before `run_started_at` and runner teardown after the assertion are outside this metric.
