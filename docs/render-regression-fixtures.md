...
The 1280×720 and 960×540 variants uniformly scale the recipe's local pixel geometry, stroke widths, grid spacing, positions and authored font sizes by 2/3 and 1/2. The bracket path starts at local (0,0), and its Transform2D position (80,68) places its cropped raster beside the cards. Root stacking retains recipe order and all items use the half-open [0,1000) ms interval. The example in [desktop hierarchy](desktop-hierarchy.md) creates the canonical variant. The dedicated native conformance compares still preview, audiovisual range preview and export at all timestamps and all three sizes, before/after a font-size edit and through undo, redo and reopen. Exact semantic plan hashes and separately reviewed PNG/PCM references live in `references.json`, bound to the recipe and the SHA-256 of the reviewed DejaVu Sans font (`ae7b7855e115a5966d8b1b3f80f254ccc117ec86f9965e202ee2940453837280`). It requires SSIM ≥ 0.99, aligned decoded float-PCM RMS ≤ 0.0001 and timing within one frame. Required native runs fail if tools or font are missing; reference capture is an ignored, review-only test.
