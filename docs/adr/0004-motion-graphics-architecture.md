...
This initial model resolves media by logical asset ID and represents an explicitly configured text font with a generated logical font-resource ID; it contains no project-relative or absolute path, URL, renderer expression, prepared file, backend command, or artifact destination. Evaluation returns the scene in a private process-local envelope beside `SceneResourceBindings`: a separate deterministic sidecar that binds logical media IDs to project-relative requests and logical font-resource IDs to the original requested path/family selection. Requested paths exist only in that sidecar. Before any collision inspection, temporary-name allocation, workspace creation, write, or process execution, a read-only media preflight validates binding closure, derives deterministic inputs, canonicalizes media, and rejects paths outside the project root. The prepared media envelope then flows into render preparation without recalculation; canonicalized paths never enter `EvaluatedScene` or a backend unchecked. Text with neither configured path nor family emits no font binding, leaving default-font selection to renderer configuration.

The scene and binding envelope are derived process-local state, so schema version 6, retained undo/redo history, request/response shapes, and the fixture-only status of `contracts/motion-graphics-v1.json` remain unchanged. Issue #13 completed the production substitution: frame preview, audiovisual range preview, materialized draft preview, and final export evaluate once, resource preparation consumes only the separate path-bearing bindings, and backend planning consumes only the closed evaluated instructions. The borrowed project-backed planning representation and its duplicate track/item traversal are removed. Ready renderers advertise this additive behavior as `evaluated_scene_rendering` through protocol version 1 capability reporting.
...
Ancestor opacity multiplies. Compositing uses premultiplied alpha in linear light, followed by conversion to the configured output color space at the output boundary. Semantic render plans for equivalent preview/export selections match exactly. Fixed synthetic rendered fixtures require visual SSIM of at least `0.99`, aligned decoded float-PCM RMS error no greater than `0.0001`, and timing alignment within one output video frame; these bounds admit codec/container effects without permitting semantic divergence.
...
The evaluated scene remains private and non-persisted. Issue #13 adds only the `evaluated_scene_rendering` capability identifier to the version-1 headless and MCP catalogs and their governed Rust and TypeScript/Zod evidence. Ownership remains governed by `contracts/contract-ownership-v1.json`. It adds no request or response field, MCP tool or resource, provider contract, stable error, schema migration, or project serialization change.
