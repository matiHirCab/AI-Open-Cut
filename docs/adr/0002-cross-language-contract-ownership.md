...
The parity gate is `bun run contracts:check` from `apps/agent-bridge`. It runs TypeScript checking plus focused Rust and TypeScript tests against the same canonical artifacts. The Rust operation check derives snake-case names from the actual headless request enum and verifies that its Serde deserializer accepts every derived wire tag. The MCP check converts every registered tool's input and output Zod schemas to draft 2020-12 JSON Schema, recursively removes description-only copy, and compares the remaining structural schemas and complete client-visible annotations with `contracts/mcp-surface-v1.json`; non-representable runtime refinements remain in focused behavioral tests. CI runs this gate explicitly, and `.github/CODEOWNERS` requests `@matiHirCab` for canonical artifacts and governed consumers.
